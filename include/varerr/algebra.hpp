...
// types are ordered by a rank function which partitions the universe into equi-
// valence classes of equally-ranked types. Rank functions must be defined as
// static member templates of a universe class which parameterizes the row oper-
// ations in this header. The intended rank functions are insensitive to cvref-
// qualifiers. However, that condition is not enforced in this header.

// Normalization and merge operations currently resolve rank collisions by posi-
// tion. The manner in which rank collisions are resolved is not guaranteed.

// Define VARERR_PACK_SUBSCRIPT_IMPL before including this header to select the
// pack subscripting implementation. The implementation falls back to a default
// implementation if the compiler does not support pack indexing at the chosen
// language standard or provide a pack indexing builtin (MSVC, AppleClang). The
// choice of implementation may vary across translation units.
...
template <typename M, typename... Es>
requires IsRankedPack<M, Es...>
[[nodiscard]] consteval bool is_normalized_pack_impl(Row<Es...>) {
...
template <typename M, typename... Es>
requires IsRankedPack<M, Es...>
inline constexpr bool is_normalized_pack_v = detail::is_normalized_pack_impl<M>(Row<Es...> {});
...
[[nodiscard]] consteval std::optional<std::size_t> pack_lookup_normalized_impl(Row<Es...>) {
...
// Determine the position of an alternative in a normalized parameter pack.

template <typename M, typename E, typename... Es>
requires IsRanked<M, E> &&
         IsNormalizedPack<M, Es...>
inline constexpr std::optional<std::size_t> pack_lookup_normalized_v =
    detail::pack_lookup_normalized_impl<M, E>(Row<Es...> {});
...
struct row_lookup_normalized_adapter;

template <typename M, typename E, typename... Es>
struct row_lookup_normalized_adapter<M, E, Row<Es...>> {
    static constexpr auto value = pack_lookup_normalized_v<M, E, Es...>;
};
...
inline constexpr std::optional<std::size_t> row_lookup_normalized_v =
    detail::row_lookup_normalized_adapter<M, E, std::remove_cvref_t<U>>::value;
...
[[nodiscard]] consteval auto pack_normalize_indices(Row<Es...>) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        if (count == 0 || ranks[indices[i]] != ranks[indices[count - 1]]) {
...
struct pack_normalize {
    ...
    // Es... that is sorted and unique with respect to M::rank. The manner in
    // which rank collisions are resolved is not guaranteed by this library.
...
// Normalize (sort and de-duplicate) a Row.
...
[[nodiscard]] consteval auto merge_normalized_rows_linear_impl(Row<Rows...>) {
    ...
    while (true) {
        ...
        const std::size_t min_rank_col = ranks_table_col[min_rank_row];
