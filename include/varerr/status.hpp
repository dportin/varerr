...
// check exact membership in the error row. An alternative E is an exact member
// of a normalized row U if E occurs in U and the alternative at the index of E
// in U is identical to E.

template <typename M, typename U, typename E>
concept IsElemExactInRow =
    IsRow<U> &&
    IsRanked<M, E> &&
    IsNormalizedRow<M, U> &&
...
struct BasicStatus final {
    ...
    // The default constructor value-initializes the first alternative.
    ...
    // jects volatile but accepts rvalue references: a visitor called on a tem-
    // porary may return an invalid pointer or reference. The noexcept specifi-
    // cation asserts nothrow invocability over the entire error row and is co-
    // nsequently more conservative than necessary.
