...
// reference is undefined if it does not refer to the active alternative. Note
// that the returned reference can dangle if the argument is a prvalue.
...
// Activate the alternative at index N in a Storage. Note that the pointer re-
// turned by std::construct_at is never null. The non-const and non-volatile
// constraints are correct because S is not deduced as a reference from an S&
// parameter (i.e., there is no reference to strip from S).
